...
@section('dashboard-actions')
    ...
    <a href="{{ route('client-portal.team-members.create') }}">
        <button class="btn btn-default pull-right">New Member</button>
    </a>
    <button id='upload-csv' class="btn btn-default pull-right">Upload CSV</button>
    {{ Form::open(['id' => 'upload-csv-form', 'route' => 'client-portal.team-members.bulk-store', 'enctype' => 'multipart/form-data']) }}
    ...
    <script>
        var DATA_TABLE_COLUMNS = [
                ...
                @can('create', new \App\User())
            {
                ...
                render: function(data, type, row, meta){
                    return  '<div class="m-0-auto d-table">' +
                        ...
                        '<input data-id="' + row.id + '" type="checkbox"' + (parseInt(row.is_active) ? 'checked' : '') +' '+ (row.role == 'admin' ? 'disabled' : '') +'>' +
                ...
                @endcan
            {
                ...
                width: '3%',
                render: function (data, type, row, meta) {
                    return '<button title="View team member" class="view btn p-0 bg-transparent m-0-auto d-block" data-id="' + row.id + '">'+
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    if(row.is_verified == 1){
                        return '<p class="text-center">Verified</p>';
                    }else{
        ...
        $(document).ready(function () {
            ...
            $('table').on('click', '.edit', function() {
                ...
                var url = '{{ route('client-portal.team-members.edit', '_ID_' ) }}';
            ...
            $('table').on('click', '.view', function() {
                var id = this.dataset.id;
                var url = '{{ route('client-portal.team-members.view', '_ID_' ) }}';
                url = url.replace('_ID_', id);
            ...
            $('table').on('click', '.verify', function() {
                var id = this.dataset.id;
                var url = '{{ route('client-portal.team-members.resend-verify', '_ID_' ) }}';
                url = url.replace('_ID_', id);
            ...
            $('table').on('change', ':checkbox', function(){
                ...
                var id = this.dataset.id;
                var url = '{{ route('client-portal.team-members.update', '_ID_' ) }}';
                url = url.replace('_ID_', id);
            ...
            function deleteTeamMember() {
                var id = this.dataset.id;
                var url = '{{ route('client-portal.team-members.delete', '_ID_' ) }}';
                url = url.replace('_ID_', id);
            ...
            $('.apply').on('click', function() {
                ...
                switch (selected){
                    case 'Delete All':
                        url = '{{ route('client-portal.team-members.bulk-delete') }}';
