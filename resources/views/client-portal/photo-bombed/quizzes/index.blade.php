...
@section('page-title', 'Master Photobomb Quiz List')
@section('dashboard-title', 'Master Photobomb Quiz List')


@section('dashboard-actions')
    @can('create', new \App\Quiz())
    <a href="{{ route('client-portal.photo-bombed.quizzes.create') }}">
        <button title="New Photobomb Quiz" class="btn btn-default pull-right">New Photobomb Quiz</button>
    </a>
    @endcan
@endsection
    ...
    <script>
            ...
            },
            {
                ...
                render: function(data, type, row, meta){
                    ...
                    return  row.games.reduce(function(html, game){
                        return html + "<a class='mr-10 fn-c-mine-shaft' href='"+ url.replace('_ID_', game.id) +"'>" + game.name + "</a>"
                    }, '');
                }
            },
                @can('create', new \App\Quiz())
        ...
        $(document).ready(function () {
            var dataTable = new DataTablesMyArcadeChef('#quiz-table', {
                ajax: '{{ route('client-portal.photo-bombed.quizzes.data-tables') }}',
            ...
            $('table').on('click', '.duplicate', function() {
                ...
                var url = '{{ route('client-portal.photo-bombed.quizzes.duplicate', '_ID_' ) }}';
            ...
            $('table').on('change', ':checkbox', function(){
                ...
                var id = this.dataset.id;
                var url = '{{ route('client-portal.photo-bombed.quizzes.update', '_ID_' ) }}';
            ...
            function deleteQuiz() {
                var id = this.dataset.id;
                var url = '{{ route('client-portal.photo-bombed.quizzes.delete', '_ID_' ) }}';
                url = url.replace('_ID_', id);
            ...
            $('.apply').on('click', function() {
                ...
                switch (selected){
                    case 'Delete All':
                        url = '{{ route('client-portal.photo-bombed.quizzes.bulk-delete') }}';
